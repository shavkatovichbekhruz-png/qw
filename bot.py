...
async def get_screenshot(message: Message, state: FSMContext):
    ...
    db.set_status(vote_id, "approved")  # admin tasdig'isiz avtomatik qabul qilinadi

    # Google Sheets sozlangan bo'lsa, shu yerga ham yozib qo'yamiz
    vote_row = db.get_vote(vote_id)
    sheets.append_row(vote_id, message.from_user.id, full_name, phone, "approved", vote_row[6])

    await message.answer("Rahmat! Ovozingiz qabul qilindi va ro'yxatga qo'shildi. ✅")
    await state.clear()

    # Adminga faqat xabar sifatida yuboriladi (tugmalarsiz, chunki avtomatik tasdiqlangan)
    caption = f"Yangi ovoz (avtomatik tasdiqlandi):\nIsm: {full_name}\nRaqam: {phone}\nID: {vote_id}"
